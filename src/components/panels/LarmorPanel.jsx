      ...
      <div className="glass-panel p-3.5 rounded-xl border border-slate-800 space-y-3">
        ...
        <div className="flex items-center justify-between text-xs font-mono pt-1">
          ...
          <div className="flex gap-1 bg-slate-900 p-0.5 rounded-lg border border-slate-800">
            <button onClick={() => setLarmorFrame('rotating')}
              className={`px-2 py-1 rounded text-[11px] ${larmorFrame === 'rotating' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' : 'text-slate-400 hover:text-white'}`}>
              Rotating
            </button>
            <button onClick={() => setLarmorFrame('lab')}
              className={`px-2 py-1 rounded text-[11px] ${larmorFrame === 'lab' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' : 'text-slate-400 hover:text-white'}`}>
              Laboratory
            </button>
          </div>
        </div>
        ...
        <div className="pt-2">
          <button onClick={toggleLarmorSimulation}
            className={`w-full py-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition ${isLarmorRunning ? 'bg-amber-700 hover:bg-amber-600 text-white' : 'bg-emerald-700 hover:bg-emerald-600 text-white'}`}>
