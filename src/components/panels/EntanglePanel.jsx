      ...
      <div className="glass-panel p-3.5 rounded-xl border border-slate-800 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 pt-2">
          <button onClick={() => { setEntanglementConcurrence(0); }}
            className="p-2 rounded-lg bg-slate-900 border border-slate-800 hover:border-cyan-500/40 text-xs font-mono text-cyan-300">
            Separable |00⟩
          </button>
          <button onClick={() => { setEntanglementConcurrence(1); }}
            className="p-2 rounded-lg bg-slate-900 border border-rose-500/30 hover:border-rose-500 text-xs font-mono text-rose-300">
            Maximal Bell State
          </button>
        </div>
        ...
        <div className="p-2.5 rounded-lg bg-slate-900/90 border border-slate-800 text-[11px] font-mono space-y-1">
          <div className="flex justify-between">
            ...
            <span className="text-cyan-300 font-bold">{r.toFixed(3)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Subsystem Entropy S(ρ_A):</span>
            <span className="text-amber-300 font-bold">{entropy.toFixed(3)} bits</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs font-mono">
        <span className="text-slate-300">Concurrence C:</span>
        <span className="text-rose-400 font-bold">{c.toFixed(2)} ({label})</span>
      </div>
